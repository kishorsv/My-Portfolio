  ...
  return (
    <div className={`max-w-[1200px] mx-auto px-6 md:px-10 lg:px-16 my-8 select-none ${className}`}>
      <div className="flex items-center gap-4 text-muted/60 font-mono text-[11px] uppercase tracking-[0.25em]">
        {/* Tiny Number */}
        <span className="text-[#89AACC] font-semibold">{number}</span>
        ...
        <motion.div
          ...
          className="flex-1 h-[1px] bg-gradient-to-r from-white/20 via-white/10 to-transparent origin-left"
        />

        {/* Micro Label */}
        <span className="tracking-[0.3em] font-medium text-muted/80">{label}</span>
      </div>
    </div>
  );
}
